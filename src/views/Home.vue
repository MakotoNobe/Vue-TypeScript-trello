<template>
  <div class="v-row d-flex">
    ...
    <draggable :animation="200" v-model="cards" class="d-flex" draggable=".item" handle=".handle">
    <div v-for="(card, index) in cards" :key="index" class="item">
      <v-card width="355px" class="ma-3" color="grey lighten-3">
        ...
        <v-container class="overflow-y-auto" style="max-height: 700px">
         <draggable :animation="200" v-model="card.items" tag="div" group="listGruop">
              <v-card class="ma-2 move-cursor" v-for="(item, i) in card.items" :key="i">
